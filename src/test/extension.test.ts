...
import { stupefyText, activate } from '../extension';
...
suite('Extension Test Suite', () => {
	suite('stupefyText function', () => {
		test('should convert en-dash to double hyphen', () => {
			...
			assert.strictEqual(stupefyText(input), expected);
		});
		...
		test('should handle empty string', () => {
			assert.strictEqual(stupefyText(''), '');
		});
	...
	suite('VS Code Command Integration', () => {
		...
		test('command should be registered with correct ID', async () => {
			...
			assert.ok(commands.includes('markdown-stupefy.stupefy'));
		});
	});
});
